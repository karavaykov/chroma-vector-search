...
- **Расширенный поиск (v1.1.0)** - regex по проиндексированному тексту, контекстные строки вокруг совпадений, потоковая выдача результатов (WebSocket и REST SSE)
...
## 🌐 Веб-интерфейс (Новое в v1.1.0)

Chroma Vector Search теперь включает встроенный Web UI для более удобного взаимодействия.

### Запуск Web UI
Веб-интерфейс автоматически раздается микросервисом API Gateway:
```bash
# Запустить микросервисы (включая API Gateway)
...
### Возможности
- **Поиск:** Выполняйте семантический, гибридный поиск или поиск по ключевым словам прямо из браузера.
- **Настройки гибридного поиска:** Регулируйте вес семантики и ключевых слов с помощью ползунков.
- **Индексация:** Запускайте индексацию кодовой базы и следите за прогрессом в реальном времени через WebSocket.
- **Подсветка синтаксиса:** Результаты кода отображаются с правильной подсветкой синтаксиса.
- **Бейджи метаданных:** Просматривайте enterprise-метаданные 1С/BSL (автор, тип модуля, вызовы функций) прямо в результатах.

### Расширенный поиск (v1.1.0)
- **Поиск по regex:** по шаблону по in-memory документам keyword-индекса (сначала выполните индексацию, чтобы индекс ключевых слов был заполнен).
- **Контекст вокруг совпадений:** параметры `context_lines` или `context_before` / `context_after` для семантического, keyword, гибридного и regex-поиска (монолит `chroma_simple_server.py` и WebSocket API).
- **Стриминг:** в Web UI включите потоковую выдачу; в WebSocket передайте `stream: true` в сообщении `search` и обрабатывайте `search_result_chunk` и финальное `search_complete`. В микросервисах: `POST /api/v1/search/stream` на API Gateway (SSE) проксируется в Search Service.

## 🌐 WebSocket API (Новое в v1.1.0)

Двусторонняя связь в реальном времени с поддержкой WebSocket:

### Возможности WebSocket
- **Результаты поиска в реальном времени**: Мгновенная потоковая передача ответов
- **Потоковая выдача по одному результату (v1.1.0):** при `data.stream: true` сервер шлёт сообщения `search_result_chunk` и завершает `search_complete` (помимо одноразового `search_results`).
- **Обновления прогресса**: Прогресс индексации в реальном времени
- **Подписка на события**: Подписка на события сервера
- **Двусторонняя связь**: Сервер может отправлять обновления
- **Низкая задержка**: По сравнению с HTTP/TCP запросами

### Быстрый старт с WebSocket
```bash
# Запуск сервера с WebSocket (порт по умолчанию 8766)
...
Подробная документация: [docs/WEBSOCKET_API.md](docs/WEBSOCKET_API.md).
...
На Windows тесты вызывают `ChromaSimpleServer.close()`, чтобы освободить файлы SQLite Chroma перед удалением временных каталогов. В своих скриптах после работы с сервером также вызывайте `close()`.
